...
from PySide6.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QSizePolicy, QHBoxLayout, QLineEdit, \
    QRadioButton, QPushButton, QTableWidget, QScrollArea, QPlainTextEdit, QStatusBar
...
for i in range(int(math.sqrt(n))):
    ...
    comLabel = QLabel("COM:")  # Changed from QWidget() to QLabel
    comNumber = QLineEdit()
...
submitButton = QPushButton("Status")
...
box_layout.addWidget(scroll_area, stretch=1)
...
topRight_layout.addWidget(spaceBottom, stretch=1)
topRight_layout.addWidget(submitButton, stretch=1)

response = QPlainTextEdit()
response.setReadOnly(True)
boxBottom_layout.addWidget(response, stretch=5)

stats = QStatusBar()
boxBottom_layout.addWidget(stats, stretch=1)
...
window.setFixedSize(1200, 600)  # Start with a reasonable size
window.show()
