...
def create_module_asset(module_model_id, module_name):
    response = client.create_asset(
        assetName=module_name,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python Module_Modified.py <ModuleName>")
        sys.exit(1)

    custom_module_name = sys.argv[1]
    module_model_id, hierarchy_id = get_module_model_info()
    create_module_asset(module_model_id, custom_module_name)
